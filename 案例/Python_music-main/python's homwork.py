...
class MainUi(QtWidgets.QMainWindow, QDialog):
    ...
    def __init__(self, username):
        ...
        self.init_ui()
        # self._conn = pymysql.connect(host='47.113.195.193', user='root', password='123456', db='pyhomework')
    ...
    def download(self):
        ...
        if filename[0] != '':
            try:
                conn = pymysql.connect(host='47.113.195.193', user='root', password='129496', db='pyhomework')
                cur = conn.cursor()
            except:
    ...
    def collect(self):
        if self.play_index_now == -1:
            ...
            return
        try:
            conn = pymysql.connect(host='47.113.195.193', user='root', password='123456', db='pyhomework')
            cur = conn.cursor()
        except:
            msg_box = QMessageBox(QMessageBox.Warning, '温馨提示', '数据库连接错误')
            msg_box.exec_()
